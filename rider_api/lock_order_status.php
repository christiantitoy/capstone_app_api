...
try {
    ...
    if (isset($_SERVER['CONTENT_TYPE']) && $_SERVER['CONTENT_TYPE'] === 'application/json') {
        ...
        $rider_id = isset($data['rider_id']) ? intval($data['rider_id']) : 0; // ✅ ADDED
    } else {
        // Regular form POST
        $order_id = isset($_POST['order_id']) ? intval($_POST['order_id']) : 0;
        $status = isset($_POST['status']) ? $_POST['status'] : '';
        $rider_id = isset($_POST['rider_id']) ? intval($_POST['rider_id']) : 0; // ✅ ADDED
    }
    ...
    // ✅ ADDED: Check rider status BEFORE locking
    if ($status === 'locked') {
        ...
        $riderStmt = $conn->prepare("SELECT status FROM riders WHERE id = :rider_id");
