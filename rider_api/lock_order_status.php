...
// ✅ Get notification title function
...
// ✅ Send notification function (calls sendNotification.php which handles save + push)
function sendOrderNotification($conn, $order_id, $status) {
    try {
        ...
        if (shouldSendNotification($status)) {
            ...
            error_log("Notification for order $order_id, status $status - Saved: " . ($saved ? 'Yes' : 'No') . ", Sent: " . ($sent ? 'Yes' : 'No'));
