...
try {
    ...
    // Official plan data (from sellers table) - used for pricing UI
    if ($official) {
    ...
    if ($subscribed && in_array($subscribed['subscribed_status'], ['active', 'pending'])) {
        ...
        if ($subscribed['start_date']) {
            $planData['start_date_formatted'] = date('M d, Y', strtotime($subscribed['start_date']));
        }
    ...
    $planData['description'] = $descriptions[$planData['subscribed_plan']] ?? $descriptions['bronze'];
